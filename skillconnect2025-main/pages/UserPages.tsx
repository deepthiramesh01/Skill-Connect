...
import { Navigation, Page, User, Project, Pronouns, ProjectStatus, UserRole, Notification } from '../types';
...
const defaultAvatar = 'data:image/svg+xml;base64,';
    ...
    return (
        <div className="flex flex-col min-h-screen">
            ...
            <main className="flex-grow">
                {/* FIX: Pass props explicitly to child components to fix type errors. */}
                ...
                {/* FIX: Add ProjectSearchPage component to resolve 'Cannot find name' error. */}
                {props.page === 'projectSearch' && <ProjectSearchPage navigate={props.navigate} />}
                {/* FIX: Add ProjectCreatePage component to resolve 'Cannot find name' error. */}
                {props.page === 'projectCreate' && <ProjectCreatePage navigate={props.navigate} />}
                {/* FIX: Add ProjectDetailPage component to resolve 'Cannot find name' error. */}
                {props.page === 'projectDetail' && <ProjectDetailPage navigate={props.navigate} params={props.params} />}
                {/* FIX: Add NotificationsPage component to resolve 'Cannot find name' error. */}
                {props.page === 'notifications' && <NotificationsPage navigate={props.navigate} />}
            </main>
...
// FIX: Implement ProjectSearchPage component
...
// FIX: Implement ProjectCreatePage component
...
// FIX: Implement ProjectDetailPage component
const ProjectDetailPage: React.FC<Navigation & { params: { projectId?: string } }> = ({ navigate, params }) => {
    ...
    const handleReviewChange = (userId: string, review: string) => {
        setTeamRatings(prev => ({ ...prev, [userId]: { ...prev[userId], review } }));
    };
...
// FIX: Implement NotificationsPage component
const NotificationsPage: React.FC<Navigation> = ({ navigate }) => {
    ...
    const handleNotificationClick = (notification: Notification) => {
        ...
        if (notification.link) {
            navigate(notification.link.page, notification.link.params);
        }
    };
                    ...
                    <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                        {userNotifications.map(n => (
                            <li key={n.id} onClick={() => handleNotificationClick(n)}
                                className={`p-4 transition-colors duration-200 ${n.link ? 'cursor-pointer' : ''} ${n.isRead ? 'hover:bg-gray-50 dark:hover:bg-gray-700/50' : 'bg-sky-50 hover:bg-sky-100 dark:bg-sky-900/50 dark:hover:bg-sky-900/80'}`}
                            >
                                <div className="flex justify-between items-start">
                                    <p className="text-gray-800 dark:text-gray-200">{n.message}</p>
                                    {!n.isRead && <span className="h-2 w-2 rounded-full bg-sky-500 flex-shrink-0 mt-1.5 ml-4"></span>}
                                </div>
                                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{new Date(n.createdAt).toLocaleString()}</p>
                            </li>
                        ))}
                    </ul>
                ) : (
